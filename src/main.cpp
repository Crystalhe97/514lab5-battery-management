...
const char* password = "]59GV#$yLm"; // Replace with your network password
#define DATABASE_URL "https://techin514yess-default-rtdb.firebaseio.com/" // Replace with your database URL
#define API_KEY "AIzaSyDDbgry-5ooK4mm_fAid5v3s0qANTwsfpY" // Replace with your API key
...
void checkForDeepSleep() {
  static unsigned long lastDetectionMillis = millis();
  if (millis() - lastDetectionMillis > 20000) {
...
void goToDeepSleep() {
  // Enter deep sleep for the specified duration (in milliseconds)
  esp_sleep_enable_timer_wakeup(12000 * 1000);  // Convert to microseconds
...
void loop() {
  ...
  if (distance < 50) {
    ...
    // No object detected within threshold for 20 seconds, check for deep sleep
    checkForDeepSleep();
  }
}
